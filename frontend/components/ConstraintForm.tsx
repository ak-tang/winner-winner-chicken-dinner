...
export default function ConstraintForm() {
  ...
  const toggleCourse = (key: CourseKey) => {
    setForm(f => ({
      ...
      courses: {
        ...f.courses,
        [key]: { ...f.courses[key], selected: !f.courses[key].selected },
      },
    }));
  };
  ...
  const setCourseQty = (key: CourseKey, qty: number) => {
    setForm(f => ({
      ...f,
      courses: { ...f.courses, [key]: { ...f.courses[key], quantity: Math.max(1, qty) } },
    }));
  };
  ...
  const canProceed = () => {
    if (step === 0) return Object.values(form.courses).some(c => c.selected);
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    const must_have_ingredients = form.must_have_ingredients
      .split(',')
      .map(s => s.trim())
      .filter(Boolean);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
      ...
      <div className="p-8">
                ...
                >
                  ...
                  {form.courses[key].selected && (
                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                      <button
                        className="w-7 h-7 rounded-full bg-green-100 text-green-800 font-bold hover:bg-green-200 flex items-center justify-center"
                        onClick={() => setCourseQty(key, form.courses[key].quantity - 1)}
                      >
                        −
                      </button>
                      <span className="w-6 text-center font-semibold text-stone-800">
                        {form.courses[key].quantity}
                      </span>
                      <button
                        className="w-7 h-7 rounded-full bg-green-100 text-green-800 font-bold hover:bg-green-200 flex items-center justify-center"
                        onClick={() => setCourseQty(key, form.courses[key].quantity + 1)}
                  ...
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                      form.courses[key].selected ? 'bg-green-600 border-green-600' : 'border-stone-300'
                    }`}
                  >
                    {form.courses[key].selected && (
        ...
        {/* Step 1: Cuisine & Occasion */}
        {step === 1 && (
          <div>
            <h2 className="text-xl font-semibold text-stone-800 mb-1">Cuisine & vibe</h2>
            <p className="text-stone-500 text-sm mb-6">What flavors are you going for? What&apos;s the occasion?</p>
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={e => setForm(f => ({ ...f, customCuisine: e.target.value }))}
                  onKeyDown={e => e.key === 'Enter' && addCustomCuisine()}
                  className="flex-1 px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400"
                />
                <button
                  onClick={addCustomCuisine}
                  className="px-4 py-2 text-sm bg-stone-100 text-stone-700 rounded-lg hover:bg-stone-200"
                >
                  Add
                </button>
              </div>
              {form.cuisines.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {form.cuisines.map(c => (
                    ...
                    >
                      {c}
                      <button onClick={() => toggleCuisine(c)} className="hover:text-green-600">×</button>
                    </span>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Birthday dinner, casual weeknight, anniversary..."
                value={form.occasion}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="mb-6">
              ...
              <p className="text-xs text-stone-400 mb-2">Comma-separated (e.g. peanuts, shellfish)</p>
              <input
                type="text"
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. truffle, lobster, seasonal tomatoes"
                value={form.must_have_ingredients}
                onChange={e => setForm(f => ({ ...f, must_have_ingredients: e.target.value }))}
                className="w-full px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400"
              />
            </div>
          </div>
        ...
        {step === 3 && (
          <div>
            ...
            <p className="text-stone-500 text-sm mb-6">How many guests and how much time do you have?</p>

            <div className="mb-6">
              <label className="block text-sm font-medium text-stone-700 mb-2">Number of guests</label>
              <div className="flex items-center gap-3">
                <button
                  className="w-9 h-9 rounded-full bg-stone-100 text-stone-800 font-bold hover:bg-stone-200 flex items-center justify-center"
                ...
                <button
                  className="w-9 h-9 rounded-full bg-stone-100 text-stone-800 font-bold hover:bg-stone-200 flex items-center justify-center"
                  onClick={() => setForm(f => ({ ...f, guest_count: f.guest_count + 1 }))}
                >
                  +
                </button>
                ...
                {(['none', 'total', 'split'] as const).map(mode => (
                  <label key={mode} className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="radio"
                      name="time_mode"
                      checked={form.time_mode === mode}
                      onChange={() => setForm(f => ({ ...f, time_mode: mode }))}
              ...
              {form.time_mode === 'total' && (
                <div className="pl-6">
                  ...
                  <input
                    ...
                    onChange={e => setForm(f => ({ ...f, total_time_minutes: Number(e.target.value) }))}
                    className="w-32 px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400"
                  />
                </div>
              )}
                ...
                <div className="pl-6 flex gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-28 px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setForm(f => ({ ...f, cook_time_minutes: Number(e.target.value) }))}
                      className="w-28 px-3 py-2 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400"
                    />
                  </div>
                </div>
        ...
        <div className="mt-8 flex gap-3 justify-between">
          <button
            ...
            disabled={step === 0}
            className="px-5 py-2.5 rounded-xl border border-stone-300 text-stone-700 text-sm font-medium hover:bg-stone-50 disabled:opacity-0 disabled:pointer-events-none"
          >
            Back
          </button>
          ...
          {step < STEPS.length - 1 ? (
            <button
              onClick={() => setStep(s => s + 1)}
              disabled={!canProceed()}
              className="px-6 py-2.5 rounded-xl bg-green-700 text-white text-sm font-medium hover:bg-green-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Continue →
            </button>
          ) : (
            <button
              ...
              disabled={loading}
              className="px-6 py-2.5 rounded-xl bg-green-700 text-white text-sm font-medium hover:bg-green-800 disabled:opacity-70 transition-colors flex items-center gap-2"
            >
              {loading ? (
                <>
